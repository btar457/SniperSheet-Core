...
import {
  ...
  ShieldAlert, Crosshair, Loader2, Target, ArrowDownToLine, Zap, Copy,
} from "lucide-react";
...
export function SmartHubTab() {
  ...
  const [openSection, setOpenSection] = useState<HubSection>("ai");
  ...
  function toggle(section: HubSection) {
    setOpenSection((prev) => (prev === section ? "ai" : section));
  }

  function handleSubmit() {
    if (!description.trim()) return;
    analyze.mutate(
      { data: { description: description.trim(), cellRef: selection?.shortAddress || undefined } },
      {
        onSuccess: (data) => {
          setLastResult(data);
  ...
  return (
    <div className="px-[10px] pt-3 pb-8 space-y-3">
      ...
      <div className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border transition-all ${
        ...
        <Crosshair className={`w-4 h-4 shrink-0 ${isWatching && selection ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"}`} />
        <div className="flex flex-col min-w-0">
          <span className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground leading-none mb-0.5">Targeting</span>
          {isWatching && selection ? (
            <span className="font-mono font-bold text-sm text-emerald-700 dark:text-emerald-300 leading-tight truncate">
              ...
              <span className="font-normal text-[11px] text-emerald-600/70 dark:text-emerald-400/70 ms-2">
                ...
                {selection.activeCellAddress !== selection.shortAddress && <> · Active: {selection.activeCellAddress}</>}
              </span>
            </span>
          ) : (
            <span className="text-xs text-muted-foreground">— حدّد خلايا في Excel / Select cells in Excel</span>
          )}
        </div>
        {isWatching && selection && (
        ...
        <div key={s.id}>
          <button
            className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg border bg-card hover:bg-muted/40 transition-colors ${
              openSection === s.id ? s.color : "border-border"
            }`}
            onClick={() => toggle(s.id)}
          >
          ...
          {openSection === s.id && (
            <div className="mt-1.5 animate-in fade-in slide-in-from-top-1 space-y-3">
                    ...
                    <CardContent className="p-4 space-y-3">
                      <div>
                        ...
                        <Textarea
                          ...
                          placeholder={"صف المنطق بالعربية أو الإنجليزية...\ne.g. If hours > 40, calculate overtime at 1.5×\nمثال: إذا الدرجة > 90 اكتب ممتاز وإلا جيد"}
                          className={`resize-none min-h-[90px] text-sm leading-relaxed transition-colors ${
                            radarIssues.some((i) => i.severity === "warning") ? "border-amber-400 dark:border-amber-600 focus-visible:ring-amber-400" : ""
                          }`}
                          dir="auto"
                          data-testid="input-smart-description"
                        />
                        {description.trim() && radarIssues.length > 0 && (
                          <div className="mt-2 rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 p-2 space-y-1.5 animate-in fade-in" data-testid="word-radar-panel">
                            ...
                            {radarIssues.slice(0, 3).map((issue, i) => (
                              <div key={i} className="text-[10px] space-y-0.5">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <code className="px-1 py-0.5 rounded text-[10px] font-mono bg-amber-200 dark:bg-amber-800 text-amber-800 dark:text-amber-200">{issue.token}</code>
                                  <span className="text-muted-foreground">{issue.reason}</span>
                                </div>
                                {issue.suggestions.length > 0 && (
                                  <div className="flex flex-wrap gap-1">
                                    {issue.suggestions.map((s) => (
                                      <button key={s} onClick={() => setDescription(description.replace(new RegExp(`\\b${issue.token}\\b`, "gi"), s))}
                      ...
                      <Button onClick={handleSubmit} disabled={analyze.isPending || !description.trim()} className="w-full h-10 font-semibold text-sm" data-testid="button-smart-analyze">
                        {analyze.isPending
                          ? <span className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />Analyzing...</span>
                          : <span className="flex items-center gap-2"><Send className="w-4 h-4" />Execute Smart Command<kbd className="ms-1 text-[10px] opacity-50 font-mono bg-primary-foreground/10 px-1 rounded">Ctrl+↵</kbd></span>}
                      </Button>

                      <button onClick={() => setShowExamples((v) => !v)} className="w-full flex items-center justify-between text-[11px] text-muted-foreground hover:text-foreground transition-colors" data-testid="button-toggle-examples">
                      ...
                      {showExamples && (
                        <div className="space-y-1 animate-in fade-in slide-in-from-top-1 -mt-1">
                          {EXAMPLE_PROMPTS.map((ex, i) => (
                            <button key={i} onClick={() => { setDescription(ex.ar); setShowExamples(false); textareaRef.current?.focus(); }}
                              className="w-full text-right text-[11px] text-muted-foreground hover:text-foreground hover:bg-muted/60 px-2 py-1.5 rounded transition-colors border border-transparent hover:border-border/50" dir="rtl">
                              {ex.ar}
                            </button>
                          ))}
                        </div>
                    ...
                    <div className="animate-in fade-in slide-in-from-bottom-2 space-y-3" data-testid="smart-result-panel">
                      <Card className="border-primary/30 shadow-sm">
                            ...
                            <div className="flex items-center gap-1.5">
                              {typeConf && (
                                <span className={`flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full font-medium ${typeConf.color}`}>
                                  {typeConf.icon}{typeConf.arLabel}
                                </span>
                              )}
                        ...
                        <CardContent className="p-3 pt-0 space-y-2">
                          <div className="font-mono text-sm bg-muted rounded-md px-3 py-2.5 border border-border/50 font-semibold break-all cursor-copy select-all"
                            onClick={() => navigator.clipboard?.writeText(lastResult.formula)} title="Click to copy" data-testid="text-smart-formula">
                            {lastResult.formula}
                          </div>
                          {lastResult.result !== null && (
                            <div className="flex items-center gap-2 px-1">
                              <span className="text-[10px] text-muted-foreground">Result:</span>
                              <span className="font-mono font-bold text-primary text-sm" data-testid="text-smart-result">{lastResult.result}</span>
                            </div>
                          )}
                          <div className="border-t border-border/50 pt-2 space-y-1.5">
                            <p className="text-[10px] text-muted-foreground font-medium">Insert into Excel:</p>
                            <div className="grid grid-cols-2 gap-1.5">
                              <Button size="sm" className="h-8 text-[11px] gap-1.5" onClick={handleInsertActive} disabled={!!insertBusy || !isWatching} data-testid="button-insert-active">
                                {insertBusy === "active" ? <Loader2 className="w-3 h-3 animate-spin" /> : <Target className="w-3 h-3" />}
                                Active Cell {selection?.activeCellAddress && <code className="ms-0.5 opacity-70 text-[10px]">{selection.activeCellAddress}</code>}
                              </Button>
                              <Button size="sm" variant="outline" className="h-8 text-[11px] gap-1.5" onClick={handleInsertInRange} disabled={!!insertBusy || !isWatching || !selection?.shortAddress} data-testid="button-insert-range">
                                {insertBusy === "range" ? <Loader2 className="w-3 h-3 animate-spin" /> : <ArrowDownToLine className="w-3 h-3" />}
                                Selection {selection?.shortAddress && <code className="ms-0.5 opacity-70 text-[10px]">{selection.shortAddress}</code>}
                              </Button>
                            </div>
                            {!isWatching && <p className="text-[10px] text-muted-foreground text-center">Open in Excel Online to enable insertion</p>}
                            {insertStatus && (
                              <div className={`flex items-center gap-1.5 text-[11px] px-2 py-1 rounded border animate-in fade-in ${
                                ...
                                {insertStatus.ok ? <CheckCircle2 className="w-3 h-3 shrink-0" /> : <AlertCircle className="w-3 h-3 shrink-0" />}
                      ...
                      <Card className="border-border/50 shadow-none bg-muted/30">
                        ...
                        <CardContent className="p-3 pt-0">
                          <p className="text-xs text-foreground leading-relaxed" dir="auto" data-testid="text-smart-reasoning">{lastResult.reasoning}</p>
                        </CardContent>
                      </Card>
                        ...
                        <Card className="border-pink-200 dark:border-pink-800 shadow-none">
                          <CardHeader className="p-3 pb-1">
                            <CardTitle className="text-[11px] font-semibold flex items-center gap-1.5">
                              <Palette className="w-3 h-3 text-pink-500" />Styling Hints
                            </CardTitle>
                          </CardHeader>
                          <CardContent className="p-3 pt-0 space-y-1.5">
                            {lastResult.styleHints.map((hint, i) => (
                              <div key={i} className="flex items-center gap-2 text-xs">
                                ...
                                {hint.condition && <span className="text-muted-foreground font-mono text-[10px] truncate">when: {hint.condition}</span>}
                              </div>
                            ))}
                          </CardContent>
                        </Card>
      ...
      <div>
        ...
        >
          <span className="flex items-center gap-1.5"><Clock className="w-3 h-3" />History ({history.length})</span>
          {showHistory ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
        </button>
        {showHistory && (
          <div className="space-y-2 animate-in fade-in">
            {history.length === 0 ? (
              <div className="text-xs text-center text-muted-foreground py-6 border border-dashed rounded-md bg-muted/30">No commands executed yet</div>
            ) : (
              history.slice(0, 10).map((item) => {
                const tc = FORMULA_TYPE_CONFIG[item.formulaType] ?? FORMULA_TYPE_CONFIG.arithmetic;
                return (
                  <div key={item.id} className="bg-card border border-border rounded-md p-2.5 hover:border-primary/30 transition-colors space-y-1.5 cursor-pointer"
                    onClick={() => { setDescription(item.description); setLastResult({ formula: item.formula, result: item.result, reasoning: item.reasoning, formulaType: item.formulaType, styleHints: item.styleHints, confidence: item.confidence }); setInsertStatus(null); setOpenSection("ai"); }}
                    data-testid={`smart-history-${item.id}`}>
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-[11px] text-foreground truncate flex-1" dir="auto">{item.description}</p>
                      <span className={`flex-none flex items-center gap-0.5 text-[9px] px-1 py-0.5 rounded-full ${tc.color}`}>{tc.icon}</span>
                    </div>
                    <div className="font-mono text-[10px] bg-muted px-1.5 py-0.5 rounded text-muted-foreground truncate">{item.formula}</div>
